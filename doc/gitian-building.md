...
*Setup instructions for a Gitian build of Alaris using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Alaris
...
- [Building Alaris](#building-alaris)
...
Clone the git repositories for alaris-core and Gitian.
...
in the alaris-core repository under 'Fetch and create inputs' to install sources which require
...
To build Alaris (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the alaris-core repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/alaris/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/alaris/alaris-core
...
COMMIT=hardfork-nmc
./bin/gbuild --commit alaris=${COMMIT} --url alaris=${URL} ../alaris-core/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit alaris=${COMMIT} --url alaris=${URL} ../alaris-core/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit alaris=${COMMIT} --url alaris=${URL} ../alaris-core/contrib/gitian-descriptors/gitian-osx.yml
```

Building fully offline
-----------------------

**Note: Alaris does not yet use detatched sigs; this section might not be relevant.**

For building fully offline including attaching signatures to unsigned builds, the detached-sigs repository
and the alaris-core git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../alaris-core/contrib/gitian-descriptors/*|sort|uniq )
...
cd /some/root/path/
git clone https://github.com/alaris/alaris-detached-sigs.git

ALAPATH=/some/root/path/alaris-core.git
SIGPATH=/some/root/path/alaris-detached-sigs.git

./bin/gbuild --url alaris=${ALAPATH},signature=${SIGPATH} ../alaris-core/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/alaris-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/alaris-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/alaris-osx-build.assert
```
...
[alaris/gitian.sigs](https://github.com/alaris/gitian.sigs/) repository, or if that's not possible create a pull
